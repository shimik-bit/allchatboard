/**
 * English translations - mirrors the Hebrew dictionary 1:1.
 *
 * MAINTENANCE: When you add a string to he.ts, add the same key here. Use
 * the LocaleDictionary type to catch missing keys at compile time.
 */

import type { LocaleDictionary } from './he';

export const en: LocaleDictionary = {
  /* ────────────── Common UI ────────────── */
  common: {
    save: 'Save',
    cancel: 'Cancel',
    delete: 'Delete',
    edit: 'Edit',
    back: 'Back',
    next: 'Next',
    close: 'Close',
    confirm: 'Confirm',
    yes: 'Yes',
    no: 'No',
    loading: 'Loading...',
    saving: 'Saving...',
    deleting: 'Deleting...',
    search: 'Search',
    filter: 'Filter',
    add: 'Add',
    remove: 'Remove',
    open: 'Open',
    create: 'Create',
    update: 'Update',
    enabled: 'Enabled',
    disabled: 'Disabled',
    active: 'Active',
    inactive: 'Inactive',
    optional: 'Optional',
    required: 'Required',
    name: 'Name',
    description: 'Description',
    notes: 'Notes',
    settings: 'Settings',
    actions: 'Actions',
    status: 'Status',
    type: 'Type',
    date: 'Date',
    time: 'Time',
    today: 'Today',
    yesterday: 'Yesterday',
    tomorrow: 'Tomorrow',
    error: 'Error',
    success: 'Success',
    warning: 'Warning',
    info: 'Info',
    not_found: 'Not found',
    no_results: 'No results',
    confirm_delete: 'Are you sure you want to delete this?',
    cannot_undo: 'This action cannot be undone',
    coming_soon: 'Coming soon',
    done: 'Done',
  },

  /* ────────────── Navigation ────────────── */
  nav: {
    overview: 'Overview',
    tables: 'Tables',
    new_table: 'New table',
    whatsapp: 'WhatsApp',
    telegram: 'Telegram',
    api_keys: 'API Keys',
    reports: 'Scheduled Reports',
    automations: 'Automations',
    assignments: 'Assignees',
    phones: 'Allowed Phones',
    settings: 'Settings',
    help: 'Help',
    docs: 'Docs',
    logout: 'Sign out',
  },

  /* ────────────── Auth & Onboarding ────────────── */
  auth: {
    login: 'Sign in',
    signup: 'Sign up',
    logout: 'Sign out',
    email: 'Email',
    password: 'Password',
    enter_email: 'Enter your email',
    enter_password: 'Enter your password',
    forgot_password: 'Forgot password?',
    send_otp: 'Send code',
    verify_otp: 'Verify code',
    enter_otp: 'Enter the code from your email',
    otp_sent: 'Code sent to your email',
    welcome_back: 'Welcome back',
    create_account: 'Create account',
    sign_in_to_account: 'Sign in to your account',
    create_free_account: 'Create a free account',
    trial_no_credit_card: '14-day trial, no credit card required',
    full_name: 'Full name',
    full_name_placeholder: 'John Doe',
    password_min_chars: 'At least 6 characters',
    signup_and_start_trial: 'Sign up and start trial',
    no_account: "Don't have an account?",
    sign_up_now: 'Sign up now',
    already_have_account: 'Already have an account?',
    login_here: 'Log in here',

  },

  onboarding: {
    welcome: 'Welcome to AllChatBoard',
    workspace_name: 'Business / workspace name',
    workspace_name_placeholder: 'Example: "Avi\'s Garage" or "Moshe Restaurant"',
    choose_template: 'Choose a template',
    choose_language: 'Choose language',
    language_hint: 'Affects UI, WhatsApp bot, AI, and reports',
    template_blank: 'Start from scratch',
    template_blank_desc: "I'll build my own tables",
    template_construction: 'Construction & remodeling',
    template_property: 'Property management',
    template_restaurant: 'Restaurant',
    template_garage: 'Auto repair shop',
    finish: 'Finish',
    setting_up: 'Setting up your workspace...',
    lets_start: "Let's get started!",
    choose_industry: 'Choose your business industries. You can pick multiple templates - and add more later.',
    choose_templates: 'Choose templates (multiple OK)',
    template_singular: 'template',
    templates_plural: 'templates',
    choose_at_least_one: 'Choose at least one template',
    create_workspace_with: 'Create workspace with',

  },

  /* ────────────── Dashboard Home ────────────── */
  dashboard: {
    overview: 'Overview',
    overview_subtitle: 'A quick look at your business',
    stat_tables: 'Tables',
    stat_records: 'Records',
    stat_messages: 'WhatsApp messages',
    resources_title: 'User guide',
    resources_desc: 'Everything you need to know — 14 articles',
    api_title: 'API for developers',
    api_desc: 'Connect the system to a website, app, or Zapier',
    pdf_title: 'Download PDF',
    pdf_desc: 'Full guide, 23 pages — for offline reading or print',
    my_tables: 'My tables',
    connect_whatsapp_link: 'Connect WhatsApp →',
    no_tables_short: 'No tables yet',
    recent_activity: 'Recent activity',
    new_record_in: 'New record in {table}',
    no_content: 'No content',
    no_activity: 'No activity yet. Connect WhatsApp to get started!',
  },

  /* ────────────── Tables & Records ────────────── */
  tables: {
    new: 'New table',
    edit: 'Edit table',
    delete: 'Delete table',
    archive: 'Archive',
    name: 'Table name',
    icon: 'Icon',
    description: 'Table description',
    fields: 'Fields',
    no_tables: 'No tables yet',
    create_first: 'Create your first table',
    add_field: 'Add field',
    primary_field: 'Primary field',
    archived: 'Archived',
    restore: 'Restore from archive',
    permissions: 'Table permissions',
    ai_keywords: 'AI keywords',
    ai_keywords_hint: 'Words that should make messages route to this table',
    default_assignee: 'Default assignee',
    new_page: {
      back: 'Back',
      title: 'New table',
      subtitle: 'Define a new table with the fields you need',
      table_name_label: 'Table name',
      table_name_placeholder: 'e.g. Suppliers',
      description_label: 'Description (optional)',
      description_placeholder: 'Helps the AI understand what this table is about',
      icon_label: 'Icon',
      fields_title: 'Fields',
      add_field: '+ Add field',
      field_name: 'Field name',
      field_type: 'Type',
      target_table: 'Target table',
      select_table: 'Select a table',
      options_label: 'Options (comma separated)',
      options_placeholder: 'e.g. New, In progress, Closed',
      remove_field: 'Remove field',
      required_field: 'Required',
      create_table: 'Create table',
      creating: 'Creating...',
      error_name_required: 'Table name is required',
      error_at_least_one_field: 'At least one field is required',
      error_field_name_required: 'All fields must have a name',
      error_relation_target: 'Field "{name}" of type relation needs a target table',
      field_types: {
        text: 'Short text',
        longtext: 'Long text',
        number: 'Number',
        currency: 'Currency (₪)',
        date: 'Date',
        datetime: 'Date + time',
        select: 'Single select',
        multiselect: 'Multi select',
        status: 'Status',
        checkbox: 'Checkbox',
        phone: 'Phone',
        email: 'Email',
        url: 'URL',
        rating: 'Star rating',
        relation: '🔗 Record link',
      },
    },
  },

  records: {
    new: 'New record',
    edit: 'Edit record',
    delete: 'Delete record',
    delete_confirm: "Delete this record? This action can't be undone.",
    no_records: 'No records yet',
    no_records_hint: 'Add a record manually or wait for them to come in via WhatsApp',
    create_first: 'Add first record',
    save: 'Save record',
    update: 'Update record',
    saved: 'Saved',
    showing: 'Showing {n} records',
    of: 'of',
    source_whatsapp: 'from WhatsApp',
    source_manual: 'created manually',
    source_api: 'via API',
    move_to_table: 'Move to table...',
    show_more_fields: 'Show more fields',
    show_less: 'Show less',
    last_updated: 'Last updated',
    created_at: 'Created',
    assignee: 'Assignee',
    no_assignee: 'No assignee',
  },

  /* ────────────── Field types ────────────── */
  fields: {
    text: 'Short text',
    longtext: 'Long text',
    number: 'Number',
    currency: 'Currency',
    date: 'Date',
    datetime: 'Date & time',
    select: 'Select',
    multiselect: 'Multi-select',
    status: 'Status',
    checkbox: 'Checkbox',
    phone: 'Phone',
    email: 'Email',
    url: 'URL',
    city: 'City',
    relation: 'Link to table',
    file: 'File attachment',
    rating: 'Rating',
    field_name: 'Field name',
    field_type: 'Field type',
    is_required: 'Required',
    ai_hint: 'AI hint',
    ai_hint_placeholder: 'e.g. "10-digit phone number"',
    options: 'Options',
    add_option: 'Add option',
    option_value: 'Value',
    option_label: 'Label',
    option_color: 'Color',
    relation_target: 'Target table',
    display_columns: 'Display columns',
    is_primary: 'Primary field',
  },

  /* ────────────── Views ────────────── */
  views: {
    table: 'Table',
    kanban: 'Kanban',
    calendar: 'Calendar',
    grid: 'Grid',
    no_status_field: 'No status field',
    no_status_field_hint: 'Kanban view requires a status field',
    no_date_field: 'No date field',
    no_date_field_hint: 'Calendar view requires a date field',
    drag_hint: 'Drag cards between columns to change status',
    drop_here: 'Drop here',
    empty_column: 'Empty',
  },

  /* ────────────── WhatsApp Settings ────────────── */
  whatsapp: {
    title: 'WhatsApp Settings',
    not_connected: 'WhatsApp not connected',
    connect: 'Connect WhatsApp',
    instance_id: 'Instance ID',
    instance_token: 'Token',
    status_connected: 'Connected',
    status_disconnected: 'Disconnected',
    test_message: 'Send test message',
    groups: 'Groups',
    group_target_table: 'Route to table',
    group_default_assignee: 'Default assignee',
    group_auto_create: 'Create records',
    group_auto_create_hint: 'Auto-create a record from each message',
    group_savings_active: '💰 Savings active — no AI calls for messages from this group',
    group_bot_replies: 'Bot replies in group',
    group_ai_hint: 'AI hint',
    group_internal_notes: 'Internal notes',
  },

  /* ────────────── Permissions ────────────── */
  permissions: {
    title: 'Table Permissions',
    general_access: 'General access',
    open: 'Open',
    open_desc: 'All members can view and edit',
    read_only_for_all: 'Read for all',
    read_only_desc: 'Everyone reads, only marked members can edit',
    restricted: 'Restricted',
    restricted_desc: 'Only marked members can see',
    per_user: 'Per-user permissions',
    per_user_hint: 'Set specific permissions for users that override the general access',
    no_members: 'No members in workspace',
    invite_member: 'Invite a new member to workspace',
    members_list_hint: 'This list shows existing workspace members. To add a new member, go to Settings.',
    full_access: 'Full access (admin)',
    edit_perm: 'Edit',
    read_perm: 'Read',
    blocked: 'Blocked',
    save_permissions: 'Save permissions',
    how_it_works: 'How it works:',
    how_it_works_desc: 'Owners and admins always see everything. WhatsApp messages routed to this table will fail to create a record if the sender lacks edit permission.',
  },

  /* ────────────── Settings ────────────── */
  settings: {
    title: 'Settings',
    workspace: 'Workspace',
    workspace_name: 'Workspace name',
    members: 'Team members',
    invite: 'Invite member',
    invite_email: 'Member email',
    role: 'Role',
    role_owner: 'Owner',
    role_admin: 'Admin',
    role_editor: 'Editor',
    role_viewer: 'Viewer',
    remove_member: 'Remove member',
    transfer_ownership: 'Transfer ownership',

    /* Language section */
    language: 'Language',
    language_current: 'Current language',
    language_change: 'Change language',
    language_hebrew: 'עברית',
    language_english: 'English',
    language_warning_title: 'Changing language affects:',
    language_warning_ui: 'UI for all workspace members',
    language_warning_bot: 'WhatsApp bot responses',
    language_warning_ai: 'AI classification language',
    language_warning_reports: 'Scheduled reports format',
    language_warning_data: 'Existing data (record names, field values) is NOT auto-translated.',
    language_save: 'Save changes',
    language_changed: 'Language updated. Refresh the page to see changes.',

    /* Timezone */
    timezone: 'Timezone',
    timezone_hint: 'Affects scheduled reports',
  },

  /* ────────────── Reports ────────────── */
  reports: {
    title: 'Scheduled Reports',
    new: 'New report',
    no_reports: 'No scheduled reports yet',
    no_reports_hint: 'Automatic reports sent to WhatsApp',
    create_first: 'Create first report',
    schedule: 'Schedule',
    recipients: 'Recipients',
    template: 'Template',
    last_run: 'Last',
    next_run: 'Next',
    run_count: 'Runs',
    run_now: 'Send now',
    run_confirm: 'Send report now?',
    sent_to: 'Sent to',
    history: 'Send history',
    no_history: 'No reports sent yet',

    /* Wizard steps */
    step_template: 'Template',
    step_schedule: 'Schedule',
    step_recipients: 'Recipients',
    step_preview: 'Preview',
    step_template_intro: 'Choose a report template',
    step_schedule_intro: 'When to send?',
    step_recipients_intro: 'Send to whom?',
    step_preview_intro: 'How the report will look in WhatsApp',

    /* Templates */
    template_open_tasks: 'Open tasks',
    template_open_tasks_desc: 'Daily reminder of incomplete records',
    template_leads_summary: 'Leads summary',
    template_leads_summary_desc: 'Leads grouped by campaign',
    template_sales_summary: 'Sales summary',
    template_sales_summary_desc: 'Total sales + period-over-period comparison',
    template_stuck_records: 'Stuck records',
    template_stuck_records_desc: 'Records with no updates in N days',

    /* Schedule */
    schedule_time: 'Time',
    schedule_days: 'Days',
    schedule_business_days: 'Weekdays (Mon-Fri)',
    schedule_every_day: 'Every day',
    add_recipient: '+ Add recipient',
    recipient_name: 'Name (optional)',
    recipient_phone: 'Phone',

    /* Greetings */
    greeting_morning: '🌅 Good morning',
    greeting_afternoon: '☀️ Good afternoon',
    greeting_evening: '🌇 Good evening',
    greeting_night: '🌙 Good night',
    open_link: 'Open in app',
  },

  /* ────────────── API Keys ────────────── */
  api_keys: {
    title: 'API Keys',
    description: 'Create and manage API keys for external integrations (websites, Zapier, Make, custom apps)',
    new: 'New key',
    no_keys: 'No API keys yet',
    no_keys_hint: 'Create your first key to connect a website, app, or Zapier to AllChatBoard',
    create_first: 'Create first key',
    name: 'Key name',
    name_placeholder: 'e.g. Wix website, Zapier integration',
    permissions: 'Permissions',
    can_read: 'Read (GET)',
    can_create: 'Create (POST)',
    can_update: 'Update (PATCH)',
    can_delete: 'Delete (DELETE)',
    table_access: 'Table access',
    all_tables: 'All workspace tables',
    expires_at: 'Expiration date',
    expires_hint: 'Leave empty for no expiration',
    notes: 'Internal notes',
    save_warning: 'Save the key now!',
    save_warning_desc: "This is the only time the key will be shown. If you lose it, you'll need to create a new one.",
    your_token: 'Your key:',
    copy: 'Copy key',
    copied: 'Copied!',
    revoke: 'Revoke key',
    revoke_confirm: 'Revoke this key?',
    delete_confirm: 'Permanently delete this key?',
    revoked: 'Revoked',
    expired: 'Expired',
    requests: 'Requests',
    last_used: 'Last used',
    docs_link: 'API documentation',
    tab_keys: 'Keys',
    tab_logs: 'Requests',
    no_logs: 'No API requests yet',
  },

  /* ────────────── Automations ────────────── */
  automations: {
    title: 'Automations',
    move_records: 'Move records',
    workflows: 'Workflows',
    sequences: 'Sequences',
    new_workflow: 'New workflow',
    new_sequence: 'New sequence',
    move_to_table: 'Move to table',
    select_target_table: 'Select target table',
    field_mapping: 'Field mapping',
    source_action: 'What to do with the original record',
    keep: 'Keep as is',
    mark_converted: 'Mark as "converted"',
    archive: 'Archive',
    delete: 'Delete',
    create_link: 'Save cross-link between records',
    converted_to: 'Converted to',
    originated_from: 'Originated from',

    /* Workflows */
    workflow_name: 'Workflow name',
    trigger: 'Trigger (when does this run?)',
    actions: 'Actions (what to do?)',
    add_action: 'Add action',
    trigger_record_created: 'Record created',
    trigger_field_changed: 'Field changed',
    trigger_status_equals: 'Status equals...',
    trigger_record_aged: 'Record aged',
    action_create_record: 'Create record in another table',
    action_update_field: 'Update field on this record',
    action_send_whatsapp: 'Send WhatsApp',
    action_add_task: 'Add task',
    action_start_sequence: 'Start sequence',
    action_notify_user: 'Notify user',

    /* Sequences */
    sequence_name: 'Sequence name',
    sequence_description: 'Description',
    add_step: 'Add step',
    step_when: 'When',
    step_immediate: 'Immediately on activation',
    step_after_days: 'After {n} days',
    step_channel: 'Channel',
    step_message: 'Message',
    step_condition: 'Condition',
    exit_conditions: 'Exit conditions',
    exit_on_reply: 'If customer replies, stop',
    exit_on_status: 'If status changes to X, stop',
    exit_on_unsubscribe: 'If "stop" is sent, stop',
  },

  /* ────────────── Errors ────────────── */
  errors: {
    generic: 'Something went wrong. Please try again.',
    network: 'Network error. Check your connection.',
    unauthorized: "You don't have permission for this action",
    not_found: 'Item not found',
    validation: 'Some fields are not filled correctly',
    save_failed: 'Save failed',
    delete_failed: 'Delete failed',
    load_failed: 'Failed to load data',
  },

  /* ────────────── Documentation Pages ────────────── */
  documentation: {
    official_guide: 'Official user guide',
    back_to_app: 'Back to app',
    need_help_title: 'Need personal help?',
    need_help_body: "We're here for any question. Response within 24 hours.",
    contact_us: 'Contact us',

    section_quickstart: 'Quick start',
    section_tables: 'Tables & data management',
    section_whatsapp: 'WhatsApp & automations',
    section_permissions: 'Permissions & team',
    section_advanced: 'Advanced features',
    section_developers: 'For developers',
    section_help: 'Help & support',

    first_5_min: 'First 5 minutes',
    basic_concepts: 'Core concepts',
    business_templates: 'Business templates',
    creating_tables: 'Creating tables',
    field_types: 'Field types',
    linking_tables: 'Linking tables',
    views: 'Views (table / kanban / calendar)',
    connecting_bot: 'Connecting the bot',
    managing_groups: 'Managing groups',
    how_ai_works: 'How AI works',
    workspace_roles: 'Workspace roles',
    table_permissions: 'Table permissions',
    scheduled_reports: 'Scheduled reports',
    api_reference: 'API reference',
    faq: 'FAQ',
    troubleshooting: 'Troubleshooting',
  },

  /* ────────────── Docs Home Page ────────────── */
  docs_home: {
    title: 'How to work\nwith AllChatBoard?',
    subtitle: 'Find the answer to every question. No more wandering.',

    card_quickstart_title: 'Start in 5 minutes',
    card_quickstart_desc: 'From the first screen to your first record - the important steps.',
    card_whatsapp_title: 'Connect WhatsApp',
    card_whatsapp_desc: 'The magic of the system - how to connect the bot in 5 minutes.',
    card_tables_title: 'Smart tables',
    card_tables_desc: 'How to build excellent tables that help you work fast.',
    card_permissions_title: 'Permissions & team',
    card_permissions_desc: 'How to give each team member exactly the right access.',

    popular_this_week: 'Popular this week',
    popular_1: 'How do I connect WhatsApp for the first time?',
    popular_2: 'Linking tables with 3 display columns',
    popular_3: 'Routing messages from group to specific table',
    popular_4: 'Hiding a table from a specific user',
    popular_5: "The bot isn't receiving messages - what to check?",

    full_pdf_title: 'Want everything in one file?',
    full_pdf_desc: 'The complete guide, 23 pages, organized and styled - for offline reading or printing.',
    download_pdf: 'Download PDF (200KB)',
  },

  /* ────────────── Homepage ────────────── */
  home: {
    nav_protection: 'Protection & Profiles',
    // Header nav (desktop only)
    nav_features: 'Features',
    nav_how: 'How it works',
    nav_pricing: 'Pricing',
    nav_faq: 'FAQ',
    cta_start_free: 'Start Free',
    cta_start_trial: 'Start free trial',
    cta_start_trial_14_days: 'Start 14-day free trial',
    cta_how_it_works: 'How does it work?',

    hero_badge: 'New: AI auto-classifies every message to the right table',
    hero_title_part1: 'Turn WhatsApp chats',
    hero_title_part2: 'into managed boards',
    hero_description: "TaskFlow AI takes every message from your business's WhatsApp groups, classifies them with AI, and automatically fills tables tailored to your industry.",

    feature1_title: 'Simple WhatsApp connection',
    feature1_desc: 'Connect your groups in 2 minutes via AllChat. We listen to every message.',
    feature2_title: 'AI that understands your business',
    feature2_desc: 'The AI learns your industry — auto repair, restaurant, real estate — and classifies each message to the right table.',
    feature3_title: 'Boards that fit you exactly',
    feature3_desc: 'Tables, kanban, calendar — every view for every data type. Build your business visually.',

    // GroupGuard section
    gg_badge: 'New in TaskFlow AI',
    gg_title_part1: 'Smart protection',
    gg_title_part2: '& member profiles',
    gg_description: 'Add the bot to your WhatsApp groups and get an AI that learns who the members are, what they do, and protects the group from spam - even without admin permissions.',

    // Tier 1: Member
    tier_member_badge: 'Bot as regular group member',
    tier_member_title: 'Insights & knowledge base',
    tier_member_subtitle: 'Regular addition to group - 30 seconds',

    // Tier 2: Admin
    tier_admin_badge: 'Bot with admin permissions',
    tier_admin_title_part1: 'Everything above +',
    tier_admin_title_part2: 'automatic protection',
    tier_admin_subtitle: 'Make the bot an admin - it guards the group for you',

    // Member features
    feat_profiles_title: 'Automatic member profiles',
    feat_profiles_desc: 'AI builds a profile for each member: name, profession, business, website, specialty',
    feat_search_title: 'Search by domain',
    feat_search_desc: 'For example: "Who here is a commercial lawyer?" - get a list',
    feat_dashboard_title: 'Dashboard & statistics',
    feat_dashboard_desc: 'Charts on group activity, active members, hot topics',
    feat_completion_title: 'Profile completeness percentage',
    feat_completion_desc: 'A percentage circle that grows as you collect more info about each member',
    feat_admin_tag_title: '🆕 Tag admins on spam',
    feat_admin_tag_desc: 'Bot detects spam and tags people you define - they handle manually',

    // Admin features
    feat_delete_spam_title: 'Automatic spam deletion',
    feat_delete_spam_desc: 'AI detects spam, ads, phishing - and deletes before members see',
    feat_remove_spammers_title: 'Remove spammers',
    feat_remove_spammers_desc: 'Repeat offenders? Automatically removed from the group',
    feat_block_country_title: 'Block country prefixes',
    feat_block_country_desc: 'Define which countries are blocked in advance (e.g. suspicious prefixes)',
    feat_global_blocklist_title: 'Global spammers database',
    feat_global_blocklist_desc: 'Spammer reported in thousands of other groups - auto-blocked for you too',
    feat_manual_report_title: 'Manual reporting via tagging',
    feat_manual_report_desc: 'Group members can tag the bot with "spam" and it will remove',

    // Comparison table
    comparison_title: 'Quick comparison',
    comparison_subtitle: 'What you get at each permission level',
    cmp_profiles: 'Automatic member profiles',
    cmp_search: 'Search by profession / domain',
    cmp_dashboard: 'Dashboard & statistics',
    cmp_log: 'Full activity log',
    cmp_admin_tag: '🆕 Tag admins on spam',
    cmp_chat_warnings: 'Automatic warnings in chat',
    cmp_delete_spam: 'Spam message deletion',
    cmp_remove_spammers: 'Remove spammers',
    cmp_block_country: 'Block country prefixes',
    cmp_global_blocklist: 'Cross-group spammer database',
    label_member: 'Member',
    label_admin: 'Admin',

    // Bottom CTA
    bottom_cta_hint: '💡 You can start basic and make the bot an admin later',

    // Hero mockup — WhatsApp-style group chat → extracted table rows
    hero_mockup_group_name: 'Sales Team — Avivim Real Estate',
    hero_mockup_group_meta: '12 members, 4 active now',
    hero_mockup_arrow_label: 'AI extracts & classifies each message',
    hero_mockup_table_label: 'Leads table — auto-updated',

    // 3 messages, 3 senders. Each message becomes one row in the table below.
    hero_mockup_msg1_sender: 'Daniel Cohen',
    hero_mockup_msg1_time: '14:32',
    hero_mockup_msg1_text: 'Looking for a 4-bedroom in Ramat Aviv, budget $1.2-1.5M',
    hero_mockup_msg2_sender: 'Michal Levi',
    hero_mockup_msg2_time: '14:35',
    hero_mockup_msg2_text: 'Have a client looking for penthouse in north TA, up to $3M',
    hero_mockup_msg3_sender: 'Avi Shemesh',
    hero_mockup_msg3_time: '14:38',
    hero_mockup_msg3_text: 'New lead: family with 2 kids, 5 bedrooms in Hod HaSharon',

    // Table column headers + extracted row values
    hero_mockup_col_name: 'Name',
    hero_mockup_col_property: 'Property',
    hero_mockup_col_area: 'Area',
    hero_mockup_col_budget: 'Budget',
    hero_mockup_row1_property: '4 bedrooms',
    hero_mockup_row1_area: 'Ramat Aviv',
    hero_mockup_row1_budget: '$1.2-1.5M',
    hero_mockup_row2_property: 'Penthouse',
    hero_mockup_row2_area: 'North TA',
    hero_mockup_row2_budget: 'Up to $3M',
    hero_mockup_row3_property: '5 bedrooms',
    hero_mockup_row3_area: 'Hod HaSharon',
    hero_mockup_row3_budget: '—',
    hero_mockup_row_status: 'New',

    // Stats section
    stats_eyebrow: 'By the numbers',
    stat1_value: '50,000+',
    stat1_label: 'Messages classified per month',
    stat2_value: '98%',
    stat2_label: 'AI classification accuracy',
    stat3_value: '100+',
    stat3_label: 'Active businesses',
    stat4_value: '24/7',
    stat4_label: 'Always-on classification',

    // How it works
    how_eyebrow: 'Simple process',
    how_title: 'How it works',
    how_subtitle: '3 steps, less than 5 minutes to your first table',
    how_step1_num: '01',
    how_step1_title: 'Connect WhatsApp',
    how_step1_desc: 'Easy connection through AllChat (Green API). Start receiving messages in under 2 minutes — no change to your team\u2019s daily workflow.',
    how_step2_num: '02',
    how_step2_title: 'AI learns & classifies',
    how_step2_desc: 'AI understands your industry, identifies patterns, and extracts the important details — names, addresses, products, status. Automatically, without rigid templates.',
    how_step3_num: '03',
    how_step3_title: 'See live dashboards',
    how_step3_desc: 'Every message becomes a record. Get dashboards, charts, and real-time alerts — without manual data entry, without copy-paste.',

    // Industries
    industries_eyebrow: 'Built for everyone',
    industries_title: 'Built for any business',
    industries_subtitle: 'TaskFlow AI learns your domain — no matter the industry',
    industry_realestate: 'Real Estate',
    industry_legal: 'Legal & Consulting',
    industry_health: 'Healthcare',
    industry_retail: 'Retail & Sales',
    industry_education: 'Education',
    industry_services: 'Customer Service',

    // Member Profiles section — dedicated showcase of the auto-built profiles
    profiles_eyebrow: 'Smart Profiles',
    profiles_title_part1: 'Get to know everyone',
    profiles_title_part2: 'who messages your groups',
    profiles_subtitle: 'TaskFlow AI scans every message and automatically builds a professional profile for each member — no surveys, no forms, no one needs to fill anything in.',

    // 9 capability cards around the profile mockup (3x3 grid on md+)
    profiles_cap_auto_title: 'Auto-built',
    profiles_cap_auto_desc: 'Profiles build themselves from a handful of messages — zero manual work, zero data entry.',
    profiles_cap_fields_title: '13 extracted fields',
    profiles_cap_fields_desc: 'Name, profession, specialization, business, website, city, skills, languages, topics, and more — from AI that understands context.',
    profiles_cap_country_title: 'Auto country detection',
    profiles_cap_country_desc: 'Each member\u2019s country is detected automatically from their phone prefix — no questions, no input needed.',
    profiles_cap_links_title: 'Professional link harvesting',
    profiles_cap_links_desc: 'LinkedIn, business sites, projects, portfolios — every link that appears in messages is collected automatically.',
    profiles_cap_summary_title: 'Smart summary per member',
    profiles_cap_summary_desc: 'AI summarizes in a few sentences who the person is, what they do, and what makes them unique — based on all their messages.',
    profiles_cap_completion_title: 'Completion meter',
    profiles_cap_completion_desc: 'Completion percentage per profile. See at a glance who you already know and who deserves more attention.',
    profiles_cap_search_title: 'Smart search',
    profiles_cap_search_desc: '"Who in the group works in Tel Aviv real estate?" — natural-language query, instant answer.',
    profiles_cap_refresh_title: 'Continuous updates',
    profiles_cap_refresh_desc: 'Each new message refines the profile. If someone mentions a new city or service — the profile updates.',
    profiles_cap_privacy_title: 'Privacy guaranteed',
    profiles_cap_privacy_desc: 'Data stays with you, encrypted at rest and in transit. No one outside your team sees the profiles.',

    // The big mockup card content
    profiles_mockup_eyebrow: 'Real example',
    profiles_mockup_name: 'Shira Avni',
    profiles_mockup_role: 'Senior Real Estate Broker',
    profiles_mockup_country_flag: '\uD83C\uDDEE\uD83C\uDDF1',
    profiles_mockup_country_name: 'Israel',
    profiles_mockup_country_label: 'Detected from +972',
    profiles_mockup_completion: 'Profile 87% complete',

    profiles_mockup_field_business: 'Business',
    profiles_mockup_value_business: 'Avni Boutique Real Estate',
    profiles_mockup_field_specialization: 'Specialization & Training',
    profiles_mockup_value_specialization: 'Luxury homes, penthouses | US Brokers Certified',
    profiles_mockup_field_languages: 'Languages',
    profiles_mockup_value_languages: 'Hebrew, English, Russian',

    profiles_mockup_links_label: 'Professional links',
    profiles_mockup_link_linkedin: 'linkedin.com/in/shira-avni',
    profiles_mockup_link_project: 'avni-realestate.co.il',
    profiles_mockup_link_other: 'instagram.com/shira_realestate',

    profiles_mockup_summary_label: 'AI Summary',
    profiles_mockup_summary_text: 'Real estate broker with 12+ years of experience, specializing in luxury properties in north Tel Aviv. Active in marketing groups, often shares market analyses and offers consulting to foreign investors. Holds a US brokerage license.',

    profiles_mockup_field_topics: 'Common topics',
    profiles_mockup_topic1: 'Mortgages',
    profiles_mockup_topic2: 'Investments',
    profiles_mockup_topic3: 'Resale homes',
    profiles_mockup_extracted_from: 'Built from 47 group messages',

    // FAQ
    faq_eyebrow: 'Common questions',
    faq_title: 'What you might be wondering',
    faq1_q: 'Is my data secure?',
    faq1_a: 'Yes. All data is encrypted at rest and in transit, stored in secure Supabase infrastructure, and no member of our team accesses it without your explicit permission. We are GDPR compliant.',
    faq2_q: 'What kind of WhatsApp does this work with?',
    faq2_a: 'Both regular WhatsApp and WhatsApp Business. Connection is via AllChat\u2019s Green API — no need to switch numbers or buy hardware.',
    faq3_q: 'How long does setup take?',
    faq3_a: 'Full deployment takes 2 to 15 minutes depending on the number of groups. QR scan takes seconds, after which the AI immediately starts learning and classifying.',
    faq4_q: 'What kinds of tables can I create?',
    faq4_a: 'Anything you need: leads, customers, appointments, inventory, support tickets, properties, contracts, incidents. We provide industry templates and full customization — fields, colors, rules.',
    faq5_q: 'Is there a free trial?',
    faq5_a: '14 days, fully featured, no credit card required. Full access — no limit on tables, records, or messages. If it doesn\u2019t fit — just don\u2019t continue.',
    faq6_q: 'What if I want to cancel?',
    faq6_a: 'One-click cancellation from the dashboard. No contracts, no penalties, no sales calls. Your data remains downloadable for 30 additional days.',

    // Pricing — placeholder values, will be updated later
    pricing_eyebrow: 'Pricing',
    pricing_title: 'Simple pricing, no surprises',
    pricing_subtitle: 'Pick the plan that fits. All plans include a 14-day free trial, no credit card required.',
    pricing_currency: '$',
    pricing_per_month: '/ month',
    pricing_per_day: '/ day',
    pricing_billed_monthly: 'Billed monthly, cancel anytime',
    pricing_most_popular: 'Most popular',
    pricing_cta: 'Start trial',
    pricing_cta_contact: 'Contact us',
    pricing_tab_personal: 'Personal',
    pricing_tab_business: 'Business',

    // Personal — single low-touch plan
    plan_personal_name: 'Personal',
    plan_personal_tagline: 'For individuals and solo entrepreneurs',
    plan_personal_price: '0.30',
    plan_personal_billing: '≈ $9/month • Cancel anytime',
    plan_personal_feat1: 'Use your existing WhatsApp number',
    plan_personal_feat2: 'No need for an extra phone number',
    plan_personal_feat3: '1 WhatsApp connection',
    plan_personal_feat4: 'Up to 1,000 messages/month',
    plan_personal_feat5: 'One-click cancellation, anytime',
    plan_personal_feat6: 'Email support',

    // Tier 1: Starter
    plan_starter_name: 'Starter',
    plan_starter_tagline: 'For small businesses getting started',
    plan_starter_price: '59',
    plan_starter_feat1: '1 WhatsApp connection',
    plan_starter_feat2: 'Up to 5 tables',
    plan_starter_feat3: 'Up to 5,000 messages/month',
    plan_starter_feat4: '1 user',
    plan_starter_feat5: 'Email support',

    // Tier 2: Pro (highlighted)
    plan_pro_name: 'Pro',
    plan_pro_tagline: 'For growing teams',
    plan_pro_price: '149',
    plan_pro_feat1: '3 WhatsApp connections',
    plan_pro_feat2: 'Unlimited tables',
    plan_pro_feat3: 'Up to 25,000 messages/month',
    plan_pro_feat4: 'Up to 5 users',
    plan_pro_feat5: 'Integrations (Zapier, Webhooks)',
    plan_pro_feat6: 'Chat support',

    // Tier 3: Business
    plan_biz_name: 'Business',
    plan_biz_tagline: 'For organizations and large businesses',
    plan_biz_price: '299',
    plan_biz_feat1: 'Unlimited WhatsApp connections',
    plan_biz_feat2: 'Unlimited users',
    plan_biz_feat3: 'Up to 100,000 messages/month',
    plan_biz_feat4: 'Full API and SLA',
    plan_biz_feat5: 'Dedicated success manager',
    plan_biz_feat6: '24/7 phone support',

    // Final CTA
    final_eyebrow: 'Ready to start?',
    final_title_part1: 'Turn your chats',
    final_title_part2: 'into business insights',
    final_subtitle: 'Join dozens of businesses already saving hours of work daily with TaskFlow AI.',
    final_cta_primary: 'Start 14-day free trial',
    final_cta_secondary: 'Talk to us',
    final_trust_signals: '14 days free • No credit card • Cancel anytime',

    // Footer
    footer_copyright: '© {year} TaskFlow AI. Powered by',
    footer_built_with: 'Built with',
    footer_in_israel: 'in Israel.',
  },

  /* ────────────── Onboarding Tour ────────────── */
  tour: {
    skip: 'Skip the tour',
    lets_go: "Let's go!",
    go_to_step: 'Go to step {n}',

    // Step 1: Welcome
    step1_title: 'Welcome to AllChatBoard 👋',
    step1_body1: 'Our system turns your WhatsApp into a smart management board. Every incoming message — AI classifies it, extracts details, and saves it as a record.',
    step1_body2: 'This short tour will teach you 4 things worth knowing before you start. Worth the 60 seconds.',
    step1_tip: 'You can skip at any step — you can return to the guide from the "Help" menu anytime.',

    // Step 2: Tables
    step2_title: 'It all starts with tables',
    step2_body1: 'Your workspace is built from <strong>tables</strong>. Each table is a category of data: customers, issues, appointments, billing.',
    step2_body2: 'Each table has <strong>fields</strong> (columns) and <strong>records</strong> (rows). Clicking on any row opens a complete file with all its details.',
    step2_tip: 'Already chose a template? Great - the important tables are ready waiting for you.',

    // Step 3: WhatsApp
    step3_title: 'Connect WhatsApp - this is the magic',
    step3_body1: 'Without WhatsApp - the system is a smart Excel. With WhatsApp - it becomes a personal assistant 24/7.',
    step3_body2: 'The team sends messages in the group as usual. The AI listens to everything in the background, classifies, and types details automatically into the right table.',
    step3_action: 'Connect WhatsApp now',
    step3_tip: "You can also start without WhatsApp and connect later. Everything at your pace.",

    // Step 4: Team
    step4_title: 'Invite the team',
    step4_body1: 'AllChatBoard is designed for teamwork. Invite colleagues, assign them roles (owner / admin / editor / viewer), and let them work together in the same space.',
    step4_body2: 'Want to give read-only access to some tables? There are <strong>granular permissions</strong> per table separately.',
    step4_action: 'Invite members',
    step4_tip: 'Working solo is fine too - you can use the system on your own.',

    // Step 5: Ready
    step5_title: "Ready? Let's fly 🚀",
    step5_body: "That's it - you have everything you need to start. We recommend doing two things now:",
    step5_action1: 'Open any table and click on a row - see what a full file looks like',
    step5_action2: 'Click "+ New record" and enter one data point - just to get used to it',
    step5_tip: 'Have a question? Email support@allchatboard.com or click "Help" in the menu.',
  },

  /* ────────────── Group Management (formerly GroupGuard) ────────────── */
  groupguard: {
    title: 'Group Management',
    subtitle: 'Broadcast, delete, monitor spam and manage WhatsApp group members',
    no_edit_permission: 'You don\'t have edit permission. Only owner/admin can change settings.',
    tabs: {
      dashboard: 'Overview',
      members: 'Group members',
      groups: 'Groups',
      broadcast: 'Broadcast & Delete',
      spam_protection: 'Spam Protection',
      prefixes: 'Blocked prefixes',
      whitelist: 'Whitelist',
      log: 'Action log',
    },
    common: {
      enabled: 'Active',
      disabled: 'Off',
      loading: 'Loading...',
      no_data: 'No data',
      cancel: 'Cancel',
      save: 'Save',
      add: 'Add',
      remove: 'Remove',
      refresh: 'Refresh',
      bot_admin: 'Bot is admin',
      bot_not_admin: 'Bot is not admin',
      bot_not_admin_warning: 'The bot is not admin in this group - it cannot delete messages or remove members',
    },
    groups: {
      title: 'Groups',
      no_groups: 'No WhatsApp groups registered yet',
      no_groups_hint: 'Groups are added automatically when the bot receives the first message from them',
      enable_hint: 'Enable spam protection on any group you want to monitor. The bot must be an admin to take action.',
      kicks_this_week: 'kicks this week',
      deletes: 'deletes',
      scan_members: 'Scan members',
      scan_tooltip: 'Scan group members and check who appears on the spammer blocklist',
      ai_detection: 'AI Detection',
      detection_layers: 'Detection layers',
      action_on_spam: 'Action on spam',
      action_warn: 'Warn',
      action_delete: 'Delete message',
      action_kick: 'Kick from group',
      action_notify_admins: 'Notify admins',
      bot_not_admin_title: 'The bot is not an admin in this group',
      bot_not_admin_desc1: 'Without admin permissions, the bot cannot delete messages or remove members.',
      bot_not_admin_desc2: 'Add your number as admin in the group and click "Refresh" to update.',
      detection_ai_label: 'AI content detection',
      detection_ai_desc: 'Smart analysis of suspicious messages (gpt-4o-mini)',
      detection_manual_label: 'Manual member tagging',
      detection_manual_desc: 'Group members tag the bot to report',
      detection_prefix_label: 'Blocked prefixes',
      detection_prefix_desc: 'Block by country code',
      detection_blocklist_label: 'Global spammer blocklist',
      detection_blocklist_desc: 'Shared across all AllChatBoard customers',
      manual_threshold_label: 'Manual tag threshold',
      manual_threshold_desc: 'How many different people must tag the bot on the same message to remove the sender',
      threshold_sensitive: '1 (sensitive)',
      threshold_lenient: '10 (lenient)',
      ai_sensitivity_label: 'AI sensitivity',
      sensitivity_low: 'Low',
      sensitivity_medium: 'Medium',
      sensitivity_high: 'High',
      enabled_since: 'Active since',
      saving: 'Saving...',
      saved: 'Saved',
      coming_soon: 'Coming soon',
      notify_admins_block: {
        title: 'Tag admins on spam detection',
        description: 'The bot will tag the people you configure instead of deleting/kicking itself',
        warning_not_admin: 'The bot is not an admin in this group - turn this option on to receive notifications instead of failed delete attempts.',
        admin_phones_label: 'Phone numbers to tag',
        admin_phones_placeholder: '972501234567',
        admin_phones_format_hint: 'International format without + (e.g. 972501234567). Numbers must be members of the group for tagging to work.',
        custom_message_show: 'Customize tag message',
        custom_message_hide: 'Hide',
        message_placeholder: '🚨 Spam detected!\nUser: {user} {userName}\nReason: {reason}\n{admins} - please handle',
        variables_label: 'Available variables',
        variable_user: 'tag the sender',
        variable_userName: 'sender name',
        variable_reason: 'reason',
        variable_admins: 'tag the admins',
        empty_default: 'Leave empty for default message',
        bot_admin_will_kick: '💡 When bot is admin: deletes/removes automatically.',
        bot_not_admin_will_tag: 'When bot is not admin: tags the numbers here with case details.',
      },
    },
    scan_modal: {
      title: 'Scan group members',
      scanning: 'Scanning group members...',
      scanning_subtitle: 'Cross-referencing with the global spammer blocklist',
      removing: 'Removing {count} members...',
      removing_subtitle: 'This may take a few seconds',
      total_members: 'Total members',
      flagged_count: 'Found in blocklist',
      group_clean: 'Group is clean!',
      group_clean_message: 'None of {total} members appear in the global spammer blocklist.',
      select_all: 'Select all',
      removable_count: '({count} removable)',
      selected_count: '{count} selected',
      admin_cannot_remove: 'Admin - cannot remove',
      confirmed_spammer: 'Confirmed spammer',
      reports: 'reports',
      in_groups: 'in groups',
      by_clients: 'by clients',
      reason: 'Reason',
      remove_button: 'Remove',
      remove_button_count: 'Remove {count} members',
      remove_button_default: 'Remove members',
      confirm_remove: 'Remove {count} members from the group?',
      done_title: 'Removal completed',
      done_subtitle_success: '{count} removed successfully',
      done_subtitle_failures: ', {failed} failed',
      failed_removals: 'Failed removals:',
      failure_hint: '💡 Common failures: bot not admin in group, or member already left',
      close: 'Close',
      cancel: 'Cancel',
      close_continues_bg: 'Close (continues in background)',
      scan_error: 'Scan error',
    },
    members: {
      title: 'Group members',
      no_profiles: 'No profiles yet',
      no_profiles_hint: 'Profiles are built automatically as group members send messages',
      no_results: 'No results found',
      total_in_db: '{count} group members in database',
      total: 'Total',
      messages: 'messages',
      last_seen: 'Last seen',
      groups: 'groups',
      description: 'Group members - profiles are built automatically from their messages.',
      auto_update_hint: 'Data refreshes automatically every 6 hours for active members.',
      search_placeholder: 'Search by profession, expertise, business, or name...',
      sort_recent: 'Recent',
      sort_active: 'Most active',
      sort_complete: 'Complete profile',
      all_groups: 'All groups',
      // Manual rescan button (kicks off AI extraction without waiting for cron)
      rescan_button: 'Scan now',
      call_action: 'Call',
      whatsapp_action: 'WhatsApp',
      rescan_running: 'Scanning...',
      rescan_tooltip: 'Run AI scan immediately on members not scanned in the last hour',
      rescan_done: 'Scanned {processed} profiles, {updated} updated',
      rescan_none: 'All profiles up to date. Check back in an hour.',
      rescan_error: 'Scan failed',
      loading: 'Loading...',
      previous: 'Previous',
      next: 'Next',
      page_of: 'Page {page} of {total}',
      groups_count: '{count} groups',
      info_collected: '{pct}% info collected',
      member_profile: 'Member profile',
      bio_title: 'Bio',
      business_title: 'Business / venture',
      links_title: 'Links',
      location_title: 'Location',
      skills_title: 'Skills',
      interests_title: 'Interests',
      notable_topics_title: 'Notable topics',
      languages_title: 'Languages',
      stats_title: 'Statistics',
      activity_days: 'Active days',
      member_of_groups: 'Member of {count} groups',
      messages_count_label: '{count} messages',
      recent_messages: 'Recent messages',
      first_seen: 'First seen',
      last_activity: 'Last activity',
      last_ai_analysis: 'Last AI analysis',
      profile_label: 'profile',
    },
    // Daily group summaries — per-group settings + history block
    summary: {
      title: 'Daily group summary',
      description: 'AI summarizes the day\u2019s messages into 5-8 key bullets',
      auto_label: 'Auto-summarize at end of day',
      auto_desc: 'A summary will be generated automatically every evening (~21:00 UTC)',
      hour_label: 'Hour:',
      hour_hint: '(UTC time)',
      whatsapp_label: 'Send the summary to WhatsApp',
      whatsapp_desc: 'In addition to the dashboard, the summary will be sent as a WhatsApp message',
      target_label: 'Phone number to receive the summary',
      target_placeholder: '972501234567',
      target_hint: 'Leave empty to send back to the group itself',
      saving: 'Saving...',
      save_failed: 'Save failed',
      recent_title: 'Recent summaries',
      run_now: 'Summarize now',
      running: 'Summarizing...',
      run_done: 'Summary generated',
      run_failed: 'Summary failed',
      skip_no_messages: 'No messages in the last 24 hours',
      skip_too_few: 'Found {text} text messages out of {total} total. Need at least {min} text messages to summarize. The rest are group joins/leaves or media without captions.',
      skip_already: 'Already summarized today',
      loading: 'Loading...',
      empty: 'No summaries yet. Click \u201CSummarize now\u201D to start.',
      no_headline: 'Daily summary',
      meta_count: '{count} points',
      meta_messages: '{count} messages',
      tag_auto: 'auto',
      tag_sent: 'sent to WhatsApp',
      send_error: 'Send error',
    },
    prefixes: {
      title: 'Blocked prefixes',
      no_prefixes: 'No blocked prefixes yet',
      add_prefix: 'Add prefix',
      prefix_label: 'Prefix',
      reason_label: 'Reason',
      add_to_blocklist: 'Add to blocklist',
      description: 'Block phone numbers by country code prefix. For example: 234 (Nigeria), 92 (Pakistan), 1 (USA).',
      country_name: 'Country name',
      country_placeholder: 'Nigeria',
      action_label: 'Action',
      action_kick: 'Kick from group',
      action_delete: 'Delete message only',
      action_warn: 'Warn (reply)',
      adding: 'Adding...',
      add_button: 'Add',
      cancel: 'Cancel',
      no_name: 'No name',
      delete_confirm: 'Delete this prefix?',
    },
    whitelist: {
      title: 'Whitelist',
      no_entries: 'No numbers in whitelist',
      no_entries_hint: 'Recommended: add yourself to avoid being removed by mistake',
      add_phone: 'Add number',
      phone_label: 'Phone',
      note_label: 'Note',
      description: 'Whitelisted numbers will never be removed from groups, even if reported or in the blocklist. Useful for admins, VIPs, or staff.',
      display_name_label: 'Display name',
      display_name_placeholder: 'Shimik (admin)',
      reason_label: 'Reason',
      reason_admin: 'Admin',
      reason_owner: 'Owner',
      reason_other: 'Other',
      adding: 'Adding...',
      add_button: 'Add',
      cancel: 'Cancel',
      delete_confirm: 'Remove this number from the whitelist?',
    },
    log: {
      title: 'Action log',
      no_actions: 'No actions yet',
      no_actions_hint: 'Actions will appear here as they happen',
      action_type: 'Action type',
      target: 'Target',
      reason: 'Reason',
      date: 'Date',
      total_actions: 'Total actions (7 days)',
      kicks: 'Group kicks',
      deletes: 'Messages deleted',
      failures: 'Failures',
      in_group: 'in group',
      confidence: 'Confidence',
      trigger_ai: 'AI',
      trigger_manual_report: 'Manual tag',
      trigger_phone_prefix: 'Prefix',
      trigger_global_blocklist: 'Blocklist',
      trigger_whitelist: 'Whitelist',
      action_kick: 'Kick',
      action_delete_message: 'Delete message',
      action_warn: 'Warn',
      action_blocklist_add: 'Added to blocklist',
      action_whitelist_skip: 'Skipped (whitelist)',
    },
    dashboard: {
      title: 'Dashboard',
      stats_today: 'Today',
      stats_week: 'This week',
      stats_month: 'This month',
      messages_scanned: 'Messages scanned',
      spam_caught: 'Spam caught',
      members_kicked: 'Members kicked',
      messages_deleted: 'Messages deleted',
      no_data_range: 'No data for this range',
      no_data_hint: 'Actions will appear here when the system detects spammers in groups',
      severity_high: 'High',
      severity_medium: 'Medium',
      severity_low: 'Low',
      confidence: 'Confidence',
      loading: 'Loading data...',
      time_range: 'Time range:',
      days: 'days',
      total_actions: 'Total actions',
      successful: 'Successful',
      daily_avg: 'Daily avg',
      actions_per_day: 'Actions per day',
      by_source: 'By detection source',
      by_action: 'By action type',
      ai_categories: 'AI categories',
      top_groups: 'Most active groups',
      top_spammers: 'Top spammers',
      legend_kicks: 'Kicks',
      legend_deletes: 'Deletes',
      legend_warns: 'Warns',
    },

    /* ────────────── Broadcast tab ──────────────
       Sub-tabs and form labels inside the "Broadcast & Delete" tab.
       Was previously hardcoded in BroadcastTab.tsx. */
    broadcast: {
      // Sub-tab labels (top of BroadcastTab)
      tab_new: 'New broadcast',
      tab_history: 'History',
      tab_deletes: 'Deletes',

      // AI compose button (toggles label depending on whether textarea has text)
      ai_compose: 'Write with AI',
      ai_improve: 'Improve with AI',

      // Form labels
      delay_label: 'Delay between messages (seconds)',

      // Compose form — validation + submit + success/error messages
      compose: {
        validation_message_required: 'Message text is required',
        validation_groups_required: 'Pick at least one group',
        validation_schedule_required: 'Pick a date and time for scheduling',
        validation_invalid_date: 'Invalid date',
        send_failed: 'Send failed',
        network_error: 'Network error',
        success_scheduled: 'Scheduled! {count} groups will receive the message starting at {when}',
        success_sent: 'Broadcast started! {count} groups queued.',
        message_placeholder: 'Write the message here... emojis and Hebrew supported 😊',
        group_search_placeholder: 'Search a group...',
        no_groups_found: 'No matching groups found',
        no_groups_active: 'No active groups. Scan groups in the',
        groups_link: 'Groups',
        unnamed_group: 'Unnamed group',
        summary_label: 'Summary:',
        schedule_button: 'Schedule broadcast',
        send_now_button: 'Send now',
        cancel_confirm: 'Cancel the broadcast? Messages already sent cannot be undone.',
        delete_all_confirm: 'Delete all messages sent in this broadcast?',
        error_label: 'Error',
        delete_job_created: 'Delete job created ({count} messages).',
      },

      // History view — list of past/active broadcast jobs
      history: {
        loading: 'Loading...',
        no_jobs: "You haven't sent any broadcasts yet.",
        status_pending: 'Pending',
        status_running: 'Running',
        status_completed: 'Completed',
        status_cancelled: 'Cancelled',
        status_failed: 'Failed',
      },

      // Deletions view — list of past/active delete jobs
      deletions: {
        cancel_confirm: 'Cancel the delete job?',
        loading: 'Loading...',
        no_jobs: 'No delete jobs.',
        no_jobs_hint: 'To delete messages, switch to the "{history_label}" tab and pick a completed broadcast.',
        history_label: 'History',
        status_pending: 'Pending',
        status_deleting: 'Deleting',
        status_completed: 'Completed',
        status_cancelled: 'Cancelled',
        status_failed: 'Failed',
        type_full_broadcast: '🗑️ Delete entire broadcast',
        type_manual: '🗑️ Manual delete',
      },

      // AI compose modal — opens when user clicks "Write with AI"
      ai_modal: {
        title: 'AI Compose',
        subtitle: 'Give me a topic — I write the message',
        close: 'Close',
        topic_placeholder: 'For example: 20% winter discount on all products until Friday',
        tone_label: 'Tone',
        tone_auto: 'Auto (3 tones)',
        tone_formal: 'Formal',
        tone_friendly: 'Friendly',
        tone_energetic: 'Energetic',
        length_label: 'Length',
        length_short: 'Short',
        length_medium: 'Medium',
        length_long: 'Long',
        topic_required: 'Write a short topic or idea',
        ai_error: 'AI error',
        network_error: 'Network error',
        pick_version_label: 'Pick a version:',
      },
    },
  },

  /* ────────────── Focus Mode ──────────────
     "Focus Mode" is the AI personal assistant page (/dashboard/focus).
     Was previously hardcoded in FocusClient.tsx. */
  focus: {
    title: 'Focus Mode',

    // Default prompts shown in the input + sent to the AI
    default_prompt: 'Focus me - what should I do today?',
    cta: 'Focus me - what should I do today by urgency?',
    input_placeholder: "Default: 'Focus me - what should I do today?' - or type something specific...",
    role_settings_button: 'Role settings',

    // Urgency labels for tasks (shown as colored pills)
    urgency: {
      urgent: 'Urgent!',
      important: 'Important',
      recommended: 'Recommended',
      idea: 'Idea',
    },

    // Hero greeting + briefing states
    greeting: 'Good morning! 👋',
    thinking_title: 'Thinking...',
    thinking_subtitle: 'Reviewing data, analyzing urgency, prioritizing',
    no_info_title: 'Not enough info',
    how_to_start: 'How do we start?',
    all_clear_title: 'All clear!',
    all_clear_subtitle: 'No urgent tasks right now. Enjoy your day ☕',

    // Errors
    briefing_error: 'Briefing generation failed',
    network_error: 'Network error',

    // Action buttons under each task — past-tense feedback after click
    actions: {
      done: '✓ Marked done',
      skip: '⊘ Skipped',
      postpone: '⏰ Postponed',
      delegate: '→ Delegated',
      add_to_table: '➕ Added to table',
    },

    // Role settings dialog (first-time setup or edit)
    role_dialog: {
      role_label: 'Role',
      role_placeholder: 'e.g. Sales manager',
      description_label: 'Description (optional)',
      description_placeholder: 'What do you mainly handle? What are your responsibilities?',
      examples_label: 'Quick examples:',
      cancel: 'Cancel',
      saving: 'Saving...',
      save: 'Save',
      save_failed: 'Save failed:',
    },

    // Quick role examples shown as one-tap buttons
    role_examples: {
      sales_manager_title: 'Sales Manager',
      sales_manager_desc: 'Owns the sales team, big leads, strategic deal closing',
      sales_agent_title: 'Sales Agent',
      sales_agent_desc: 'New leads, sales calls, follow-ups on offers',
      technician_title: 'Service Technician',
      technician_desc: 'Service calls, customer troubleshooting, fix documentation',
      project_manager_title: 'Project Manager',
      project_manager_desc: 'Schedules, vendor coordination, task tracking',
    },

    // Add-to-tasks dialog (when user clicks ➕ on a briefing task)
    add_to_tasks: {
      table_required: 'Please select a table',
      add_failed: 'Add failed',
      loading_options: 'Loading options...',
      load_failed: 'Load failed',
      select_table: '— Select a table —',
      assignee_self: 'Me',
      assignee_default: 'Default',
      team_member_label: 'Team member',
      select_team_member: '— Select team member —',
      allowed_phone_label: 'Allowed phone',
      select_phone: '— Select phone —',
    },
  },

  /* ────────────── Hub & CRM Modules ────────────── */
  hub: {
    title: 'Control Center - Hub',
    subtitle: 'Central control for all modules and Workspaces',
    nav_overview: 'Overview',
    nav_crm: 'CRM - Leads',
    nav_kanban: 'Kanban (drag & drop)',
    nav_buildbot: 'BuildBot - Construction',
    nav_restobot: 'RestoBot - Restaurant',
    nav_inbox_insights: 'Insights',
    nav_inbox_reports: 'Reports',
    badge_new: 'NEW',
    section_dashboards: 'Live Dashboards',
    section_workspaces: 'All Workspaces',
    section_packs: 'Available Modules',
    stats_workspaces: 'Active Workspaces',
    stats_packs_available: 'Available Modules',
    stats_installs: 'Installations',
    stats_records: 'Records',
    pack_tables: 'Tables',
    pack_fields: 'Fields',
    pack_installs: 'Installs',
    workspace_packs_count: 'Packs',
    workspace_tables_count: 'Tables',
    workspace_records_count: 'Records',
    back_to_hub: 'Back to Hub',
    no_workspace: 'No active workspace',
    crm_title: 'CRM Dashboard',
    crm_subtitle: 'Leads, pipeline, AI scores',
    crm_view_dashboard: 'Dashboard',
    crm_view_kanban: 'Kanban (drag)',
    crm_view_leads: 'All Leads',
    crm_see_all_leads: 'See all',
    crm_kpi_active_leads: 'Active Leads',
    crm_kpi_pipeline_value: 'Pipeline Value',
    crm_kpi_conversion_rate: 'Win Rate',
    crm_kpi_avg_ai_score: 'Avg. AI Score',
    crm_pipeline_status: 'Pipeline Status',
    crm_top_interactions: 'Most Active Leads',
    crm_no_leads: 'No leads with data',
    crm_calls_count: 'calls',
    crm_lead_sources: 'Lead Sources',
    crm_avg_score: 'avg. score',
    stage_new: 'New',
    stage_contacted: 'Contacted',
    stage_qualified: 'Qualified',
    stage_proposal: 'Proposal Sent',
    stage_negotiation: 'Negotiation',
    stage_won: 'Won',
    stage_lost: 'Lost',
    source_referral: 'Referral',
    source_website: 'Website',
    source_google: 'Google',
    source_whatsapp: 'WhatsApp',
    source_facebook: 'Facebook',
    source_instagram: 'Instagram',
    source_cold_call: 'Cold Call',
    source_other: 'Other',
    kanban_title: 'Leads Kanban',
    kanban_subtitle: 'Drag cards between stages to update status',
    kanban_back_to_crm: 'Back to CRM',
    kanban_add_lead: 'Add Lead',
    kanban_no_leads_in_stage: 'No leads in this stage',
    kanban_drag_here: 'Drag here',
    kanban_total: 'Total',
    kanban_no_leads: 'No leads yet',
    kanban_add_first: 'Add your first lead to get started',
    kanban_pack_not_installed: 'CRM Pack Not Installed',
    kanban_install_message: "You haven't installed the CRM pack in this workspace yet.",
    kanban_hint_long_press: 'Hold a card for a second before dragging',
    kanban_hint_double_click: 'Double-click a card to open full details',
    kanban_update_failed: 'Update failed',
    kanban_network_error: 'Network error - please retry',
    kanban_no_title: 'Untitled',
    kanban_open_card: 'Double-click to open',
    add_lead_title: 'Add New Lead',
    add_lead_field_title: 'Lead Title',
    add_lead_field_title_placeholder: 'e.g., 4-bedroom apartment on Herzl St.',
    add_lead_field_contact: 'Contact Name',
    add_lead_field_contact_placeholder: 'John Doe',
    add_lead_field_phone: 'Phone',
    add_lead_field_email: 'Email',
    add_lead_field_value: 'Opportunity Value',
    add_lead_field_stage: 'Stage',
    add_lead_field_source: 'Lead Source',
    add_lead_field_notes: 'Notes',
    add_lead_field_notes_placeholder: 'Additional info about this lead...',
    add_lead_required_title: 'Title is required',
    add_lead_create: 'Create Lead',
    add_lead_creating: 'Creating...',
    add_lead_failed: 'Failed to create lead',
    add_lead_required_marker: 'required',
    lead_not_found_title: 'Lead Not Found',
    lead_not_found_subtitle: 'It may have been deleted or never existed',
    lead_back_to_kanban: 'Back to Kanban',
    lead_updated_ago: 'Updated',
    lead_no_title: 'Untitled',
    lead_value_label: 'Opportunity Value',
    lead_change_stage: 'Current stage - click to change:',
    lead_kpi_ai_score: 'AI Score',
    lead_kpi_interactions: 'Interactions',
    lead_kpi_calls: 'Calls',
    lead_kpi_messages: 'Messages',
    lead_notes_label: 'Notes',
    lead_timeline_title: 'Timeline',
    lead_no_interactions: 'No interactions with this lead yet',
    lead_call: 'Phone Call',
    lead_sms_outbound: 'SMS Sent',
    lead_sms_inbound: 'SMS Received',
    lead_outbound: 'outbound',
    lead_inbound: 'inbound',
    buildbot_title: 'BuildBot Dashboard',
    buildbot_subtitle: 'Construction project management',
    buildbot_kpi_active_projects: 'Active Projects',
    buildbot_kpi_total_budget: 'Total Budget',
    buildbot_kpi_total_units: 'Total Units',
    buildbot_kpi_avg_size: 'Avg. Size (sqm)',
    buildbot_active_projects: 'Active Projects',
    buildbot_no_projects: 'No projects',
    buildbot_units: 'units',
    buildbot_sqm: 'sqm',
    buildbot_boq_title: 'BOQ - Cost by Chapter',
    buildbot_no_boq: 'No BOQ',
    buildbot_top_vendors: 'Top Vendors',
    buildbot_has_insurance: 'Insured',
    buildbot_no_insurance: 'No insurance',
    proj_planning: 'Planning',
    proj_permits: 'Permits',
    proj_execution: 'Execution',
    proj_finishing: 'Finishing',
    proj_completed: 'Completed',
    proj_on_hold: 'On Hold',
    chapter_earthworks: 'Earthworks',
    chapter_concrete: 'Concrete',
    chapter_masonry: 'Masonry',
    chapter_plaster: 'Plaster',
    chapter_flooring: 'Flooring',
    chapter_painting: 'Painting',
    chapter_plumbing: 'Plumbing',
    chapter_electrical: 'Electrical',
    chapter_aluminum: 'Aluminum',
    chapter_doors: 'Doors',
    chapter_kitchen: 'Kitchen',
    chapter_bathroom: 'Bathroom',
    restobot_title: 'RestoBot Dashboard',
    restobot_subtitle: 'Restaurant management - menu, inventory, reservations',
    restobot_kpi_active_dishes: 'Active Dishes',
    restobot_kpi_to_reorder: 'To Reorder',
    restobot_kpi_shifts_today: "Today's Shifts",
    restobot_kpi_reservations_today: "Today's Reservations",
    restobot_kpi_guests_today: "Today's Guests",
    restobot_revenue_potential: 'Est.',
    restobot_reservations_today: "Today's Table Reservations",
    restobot_no_reservations: 'No reservations today',
    restobot_guests: 'guests',
    restobot_table: 'table',
    restobot_staff_today: 'Staff Today',
    restobot_no_shifts: 'No shifts today',
    restobot_inventory_alerts: 'Inventory Alerts',
    restobot_no_alerts: 'Inventory looks good',
    restobot_supplier: 'Supplier',
    restobot_unknown: 'unknown',
    restobot_top_dishes: 'Top Dishes',
    restobot_tips: 'Tips',
    restobot_cost_label: 'Cost',
    occasion_birthday: 'Birthday',
    occasion_anniversary: 'Anniversary',
    occasion_business: 'Business Meeting',
    occasion_date: 'Date Night',
    occasion_family: 'Family',
    occasion_other: 'Event',
    res_confirmed: 'Confirmed',
    res_pending: 'Pending',
    res_seated: 'Seated',
    res_completed: 'Completed',
    res_cancelled: 'Cancelled',
    role_chef: 'Chef',
    role_sous_chef: 'Sous-Chef',
    role_cook: 'Cook',
    role_waiter: 'Waiter',
    role_bartender: 'Bartender',
    role_host: 'Host',
    role_cleaner: 'Cleaner',
    role_manager: 'Manager',
    cat_starter: 'Starters',
    cat_main: 'Mains',
    cat_side: 'Sides',
    cat_dessert: 'Desserts',
    cat_drinks: 'Drinks',
    cat_alcohol: 'Alcohol',
    cat_kids: 'Kids',
    buildbot_plans: 'Construction Plans',
  },

  /* ────────────── BuildBot - BOQ Generator ────────────── */
  buildbot: {
    plans_page_title: 'Construction Plans',
    plans_page_subtitle: 'Upload plans and auto-generate Bills of Quantities',
    plans_count: '{n} plans',
    plans_empty_title: 'No plans yet',
    plans_empty_subtitle: 'Upload plans (PDF, JPG, PNG, DWG) so AI can generate a BOQ',
    untitled_project: 'Untitled project',

    plan_upload_button: 'Upload Plans',
    plan_upload_title: 'Upload Construction Plans',
    plan_upload_drag: 'Drop files here or click to select',
    plan_upload_supported: 'PDF, JPG, PNG, DWG · Up to 50MB per file · Up to 10 files',
    plan_upload_pick_project: 'Assign to project',
    plan_upload_no_project: 'No project',
    plan_upload_preview_title: 'Review before upload',
    plan_upload_preview_summary: '{n} files · {size} total',
    plan_upload_start: 'Start upload',
    plan_upload_in_progress: 'Uploading plans...',
    plan_upload_done: 'Upload complete',
    plan_upload_failed: 'Upload failed',

    plan_upload_too_large: 'File too large (max 50MB)',
    plan_upload_invalid_type: 'Unsupported file type',
    plan_upload_max_files: 'Maximum {n} files at a time',

    plan_extracted_floors: 'Floors detected',
    plan_extracted_rooms: 'Rooms detected',
    plan_extracted_area: 'Total area',
    plan_confidence: 'Confidence',

    plan_status_uploaded: 'Uploaded',
    plan_status_analyzing: 'Analyzing',
    plan_status_analyzed: 'Analyzed',
    plan_status_failed: 'Failed',

    plan_upload_analyzing: 'AI is analyzing the plan...',
    plan_analysis_eta: 'Usually takes 10–60 seconds',
    plan_analysis_done: 'Analysis complete',
    plan_analysis_timeout: 'Analysis is taking too long, try refreshing',
    plan_reanalyze: 'Re-analyze',
    plan_type_label: 'Plan type',
    plan_type_floor_plan: 'Floor plan',
    plan_type_elevation: 'Elevation',
    plan_type_section: 'Section',
    plan_type_site_plan: 'Site plan',
    plan_type_detail: 'Detail',
    plan_type_other: 'Other',

    plan_rooms_breakdown: 'Detected rooms',
    plan_rooms_saved: '{n} rooms saved to table',
    plan_open_in_table: 'Open in table & edit',
    plan_no_table_yet: 'Plan rooms table not yet installed in workspace',
    floor_short: 'Floor',
  },

  /* ────────────── Inbox Insights ────────────── */
  inbox_insights: {
    title: 'Inbox Insights',
    subtitle: 'Full analytics and CRM conversion',
    back_to_inbox: 'Back to Inbox',
    kpi_urgent_active: 'Active Urgent',
    kpi_open: 'Open Tickets',
    kpi_avg_response: 'Avg. Response Time',
    kpi_resolution_rate: 'Resolution Rate',
    kpi_resolution_sub: 'resolved out of',
    last_24h: 'Last 24 Hours',
    last_7d: 'Last 7 Days',
    avg_resolution: 'Avg. Resolution Time',
    new_escalations: 'new tickets',
    open_to_resolved: 'open to resolved',
    trend_title: 'Trend - Last 14 Days',
    no_data: 'No data',
    legend_normal: 'Normal',
    legend_urgent: 'Urgent',
    by_reason_title: 'Breakdown by Reason',
    top_customers_title: 'Top Reporting Customers',
    is_lead_badge: 'CRM Lead',
    customer_escalations: 'tickets',
    customer_urgent: 'urgent',
    customer_open: 'open',
    convert_button: 'Convert to Lead',
    converting: 'Converting...',
    to_crm: 'To CRM',
    not_found_escalation: 'No escalation found to convert',
    convert_failed: 'Conversion failed',
    network_error: 'Network error',
    lead_already_exists: 'Lead already exists in CRM',
    lead_created: 'Lead created successfully!',
  },

  /* ────────────── Inbox Reports ────────────── */
  inbox_reports: {
    title: 'Inbox Reports',
    subtitle: 'SLA, performance, trends, and top issues',
    export_csv: 'Export CSV',
    exporting: 'Exporting...',
    insights: 'Insights',
    back: 'Back',
    section_sla: 'SLA & Performance',
    section_times: 'Response & Resolution Times',
    section_week_compare: 'Week-over-Week',
    section_top_issues: 'Top Issues',
    section_heatmap: 'Peak Hours - When Tickets Arrive',
    section_trend: 'Trend - Last 14 Days',
    sla_4h: 'SLA 4 Hours',
    sla_4h_sub: 'resolved within 4h',
    sla_24h: 'SLA 24 Hours',
    sla_24h_sub: 'resolved within 24h',
    resolution_rate: 'Resolution Rate',
    resolution_rate_sub: 'out of',
    urgent_open: 'Urgent Open',
    urgent_open_sub: 'require immediate attention',
    badge_good: 'Good',
    badge_improve: 'Improve',
    response_time_title: 'First Response Time (Pickup)',
    avg_label: 'Average',
    p50_label: 'Median (P50)',
    p50_hint: '50% of tickets answered within this time',
    p90_label: 'P90 (worse than average)',
    p90_hint: 'Only 10% wait longer',
    resolution_time_title: 'Full Resolution Time',
    avg_general: 'Overall Average',
    urgent: 'Urgent',
    normal: 'Normal',
    volume: 'Ticket Volume',
    urgent_count: 'Urgent',
    resolved_count: 'Resolved',
    avg_resolution_time: 'Avg. Resolution Time',
    this_week: 'This Week',
    last_week: 'Last Week',
    improvement: 'improvement',
    no_change: 'no change',
    deterioration: 'worse',
    top_issues_subtitle: 'Inquiry reasons - by volume and handling performance',
    issue_total: 'total',
    issue_urgent: 'urgent',
    issue_resolved: 'resolved',
    issue_of_total: 'of all',
    issue_resolution_rate: 'resolution',
    peak: 'Peak',
    peak_unit: 'tickets',
    heatmap_less: 'Less',
    heatmap_more: 'More',
    trend_new: 'New tickets',
    trend_resolved: 'Resolved',
    open_label: 'open',
    open_label_plural: 'open',
    reason_customer_request: 'Customer Request',
    reason_ai_uncertain: 'AI Uncertain',
    reason_complaint: 'Complaint',
    reason_payment: 'Payment',
    reason_technical: 'Technical',
    reason_human_handoff: 'Human Handoff',
    reason_bot_failed: 'Bot Failed',
    reason_other: 'Other',
    day_sun: 'Sun',
    day_mon: 'Mon',
    day_tue: 'Tue',
    day_wed: 'Wed',
    day_thu: 'Thu',
    day_fri: 'Fri',
    day_sat: 'Sat',
    status_open: 'Open',
    status_in_progress: 'In Progress',
    status_resolved: 'Resolved',
    status_dismissed: 'Dismissed',
    priority_urgent: 'Urgent',
    priority_normal: 'Normal',
    priority_low: 'Low',
  },

  /* ────────────── Inbox Extra ────────────── */
  inbox_extra: {
    convert_to_lead: 'Convert to Lead',
    converting: 'Creating...',
    convert_title: 'Create a new CRM lead from this inquiry',
    open_link: 'Open →',
    insights_link: 'Insights',
    insights_title: 'Insights & Analytics',
    reports_link: 'Reports',
    reports_title: 'Advanced reports - SLA, percentiles, heatmap',
    convert_already_exists: 'Lead already exists',
    convert_created: 'Lead created!',
    convert_failed: 'Conversion failed',
    convert_network_error: 'Network error',
  },

  /* ────────────── Time Units ────────────── */
  time_units: {
    just_now: 'just now',
    minutes_ago: '{n}m ago',
    hours_ago: '{n}h ago',
    days_ago: '{n}d ago',
    seconds: 'seconds',
    minutes: 'minutes',
    hours: 'hours',
    days: 'days',
  },

  /* ────────────── GroupGuard Onboarding Tour ────────────── */
  groupguardTour: {
    next: 'Next',
    prev: 'Back',
    skip: 'Skip tour',
    finish: 'Done 🎉',
    start_tour: 'Start guided tour',
    read_full_guide: 'Full guide',
    step_count: 'Step {current} of {total}',
    steps: {
      welcome: {
        title: '🛡️ Welcome to Group Management',
        body: 'In this short tour (about a minute) we\'ll show you how to manage your WhatsApp groups, detect spammers, and get to know your group members. You can skip at any time.',
      },
      tabs: {
        title: 'The main tabs',
        body: 'Each tab has a different role. We\'ll cover each one briefly — afterwards you can return to anywhere.',
      },
      dashboard: {
        title: '📊 Overview',
        body: 'A quick look: how many active members, messages today, recent protection actions, and a weekly chart of the busiest groups.',
      },
      broadcast: {
        title: '📣 Broadcast & Delete',
        body: 'Send messages to all groups at once, or bulk-delete. Useful for updates, promotions, or cleaning up spam in volume.',
      },
      groups: {
        title: '👥 Groups',
        body: 'See all groups the bot is in. Toggle protection on per group. You can set a different protection level for each one.',
      },
      members: {
        title: '🧑 Group Members',
        body: 'The bot automatically builds a profile for each person: profession, field, communication style. Search in plain language ("contractor in Petah Tikva").',
      },
      prefixes: {
        title: '🌍 Blocked Prefixes',
        body: 'Block numbers by country code. Most spam in Israeli groups comes from foreign prefixes — one block prevents thousands of spammers.',
      },
      whitelist: {
        title: '⭐ Whitelist',
        body: 'People who are always allowed — VIP customers, partners, team. They won\'t be blocked even if their prefix is blocked or they\'re in the global spam database.',
      },
      log: {
        title: '📋 Activity Log',
        body: 'Every bot action is recorded here with time and reason. If the bot removed a real member by mistake — you can undo the action from here.',
      },
      help: {
        title: '❓ Help is always available',
        body: 'The ? button at the top restarts this tour anytime, and links to the full guides. Good luck!',
      },
    },
  },

  /* ────────────── Telegram ────────────── */
  telegram: {
    title: 'Telegram',
    description: 'Connect Telegram bots to receive and send messages',
    workspaceLabel: 'Workspace',
    connectedBots: 'Connected Bots',
    addBot: 'Add Bot',
    addFirstBot: 'Connect your first bot',
    emptyTitle: 'No bots connected',
    emptyDescription: 'Create a Telegram bot via BotFather and connect it here to start receiving messages',
    actions: 'Actions',
    disable: 'Disable',
    enable: 'Enable',
    delete: 'Delete',
    confirmDelete: 'Delete bot @{username}? It will be removed from Telegram and stop receiving messages.',
    botDeleted: 'Bot deleted successfully',
    deleteFailed: 'Failed to delete',
    updateFailed: 'Failed to update',
    status: {
      active: 'Active',
      inactive: 'Inactive',
      error: 'Error',
    },
    add: {
      title: 'Connect Telegram Bot',
      description: 'Create a new Telegram bot and paste its token here',
      howToTitle: 'How to create a bot?',
      step1: 'Open BotFather in Telegram',
      step2: 'Send /newbot and choose a name and username',
      step3: 'BotFather will give you a token (looks like 123456:ABC...)',
      step4: 'Paste the token in the field below',
      openBotFather: 'Open BotFather in Telegram',
      tokenLabel: 'Bot Token',
      tokenHint: 'Token will be encrypted at rest and never shown again.',
      showToken: 'Show token',
      hideToken: 'Hide token',
      cancel: 'Cancel',
      connect: 'Connect Bot',
      success: 'Bot @{username} connected successfully!',
      partialSuccess: 'Bot saved but webhook setup failed: {error}',
      failed: 'Connection failed',
    },
  },
};
